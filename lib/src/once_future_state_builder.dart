...
/// future. The future will only be rebuilt if the key changes. This is useful when you want to create the future in the
/// build method of a widget, but you don't want to rebuild the future every time the widget is rebuilt.
/// e.g. If you want the future to be rebuilt anytime a parameter changes, you can use those parameters as the key (A,B,C,...).
...
class OnceFutureStateBuilderState<T> extends State<OnceFutureStateBuilder<T>> {
  ...
  Widget build(BuildContext context) {
    return FutureStateBuilder(future: future, builder: widget.builder, initialData: widget.initialData, waitingTimeoutAction: widget.waitingTimeoutAction);
  }
}